...
Node*solve(int *pre,int n,int mi,int mx,int&id)
{
    if(id==n || pre[id]<mi || pre[id]>mx)
    return NULL;
    ...
    root->left=solve(pre,n,mi,curr,id);
    root->right=solve(pre,n,curr,mx,id);
...
Node* post_order(int pre[], int size)
{
    ...
    int idx=0;
    return solve(pre,size,INT_MIN,INT_MAX,idx);
    //code here
}
